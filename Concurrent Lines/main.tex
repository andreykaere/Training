...
Пусть имеется система \(\mathfrak{M}\) материальных точек  
\(A_1(m_1), \, A_2(m_2),
\, ... \, , \, A_n(m_n)\) c центром масс \(Z\) и 
\(\exists A_z(m_z) \in \mathfrak{M} \colon Z = A_z\). Тогда, говоря в
дальнейшем о системе \(\mathfrak{M}\), мы будем подразумевать 
\textit{редуцированную} систему \(\mathfrak{M'} \colon A_z(m_z) \notin
...
\begin{note}
    ...
    не имеет смысла, поэтому это ограничение на сумму  масс существенно.
    Физически это можно понимать так:
    \textbf{<ДОПИСАТЬ ПРО ЭТО !!!>}
    \par}
\end{note}
...
\begin{proof}
	...
	Поместим массы \(1\), 
    %\(\frac{\overline{CA_1}}{\vphantom{\overline{AB'}}\overline{A_1B}}\), 
	\scalebox{0.8}{
